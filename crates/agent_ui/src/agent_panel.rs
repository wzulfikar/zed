...
impl AgentPanel {
    ...
    fn render_toolbar(&self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        h_flex()
            ...
            .when(!has_tabs || self.overlay_view.is_some(), |this| {
                this.border_b_1()
            })
            .child(
                h_flex()
                    ...
                    .when(
                        self.tabs.is_empty() || self.overlay_view.is_some(),
                        |this| this.child(self.render_title_view(window, cx)),
                    )
                    .when(
                        !self.tabs.is_empty() && self.overlay_view.is_none(),
                        |this| this.child(self.render_tab_bar(window, cx)),
                    ),
            )
            .child(
                h_flex()
