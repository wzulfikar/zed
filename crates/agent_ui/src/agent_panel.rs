...
use project::{
    AgentServerStore, ExternalAgentServerName,
...
use crate::agent_panel_tab::{AgentPanelTab, TabId, TabLabelRender};
...
impl AgentPanel {
    ...
    fn set_active_tab_by_id(&mut self, new_id: TabId, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if let Some(tab) = self.tabs.get(new_id) {
            if self.selected_agent != *tab.agent() {
                self.selected_agent = tab.agent().clone();
                self.serialize(cx);
            }
        }
    ...
    fn remove_tab_by_id(&mut self, id: TabId, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.tabs.get(id).is_some() {
            self.tabs.remove(id);
            let new_id = if self.active_tab_id == id {
                id.min(self.tabs.len() - 1)
...
impl AgentPanel {
    ...
    fn render_panel_options_menu(
        ...
        let text_thread_with_messages = match self.active_view() {
            ActiveView::TextThread {
                ...
                .read(cx)
                .messages(cx)
                .any(|message| message.role == language_model::Role::User),
            _ => false,
        };
    ...
    fn should_render_trial_end_upsell(&self, cx: &mut Context<Self>) -> bool {
        ...
        match self.active_view() {
            ...
            | ActiveView::History { .. }
            | ActiveView::Configuration
            | ActiveView::Uninitialized => return false,
        }
    ...
    fn display_tab_label(text: SharedString, is_active: bool) -> (String, Option<SharedString>) {
        const MAX_LABEL_LENGTH: usize = 30;
        let label_text = if text.len() > MAX_LABEL_LENGTH {
            format!("{}…", &text[..MAX_LABEL_LENGTH])
        } else {
            text.to_string()
        };
        let tooltip = if label_text != text.to_string() {
            Some(text)
        } else {
            None
        };
    ...
    fn render_tab_label(
        ...
        match view {
            ActiveView::ExternalAgentThread { thread_view } => {
                ...
                let (label_text, tooltip) = Self::display_tab_label(text.into(), is_active);
                    ...
                    TextThreadSummary::Content(summary) => {
                        if summary.done {
                            ...
                            let (label_text, tooltip) =
                                Self::display_tab_label(text.into(), is_active);
            ...
            ActiveView::Uninitialized | ActiveView::Configuration => TabLabelRender {
    ...
    fn render_tab_bar(&self, window: &mut Window, cx: &mut Context<Self>) -> AnyElement {
        ...
        // Get external agent names upfront to avoid borrow issues in closures
        let external_agent_names: Vec<_> = agent_server_store
            .read(cx)
            .external_agents()
            .filter(|name| {
        ...
        // If we have overlay view (History/Configuration), render it
        if let Some(overlay_view) = &self.overlay_view {
            let overlay_title = match overlay_view {
                ActiveView::History { .. } => "History",
                ActiveView::Configuration => "Settings",
                _ => "",
            };
            let title_element = h_flex()
                .flex_grow()
                ...
                .child(Label::new(overlay_title).truncate().into_any_element())
                .into_any_element();

            return tab_bar.child(title_element).into_any_element();
        }
        ...
        for (index, tab) in self.tabs.iter().enumerate() {
            ...
            // Get tab label text
            let label_text = match tab.view() {
                ActiveView::ExternalAgentThread { thread_view } => {
                    thread_view.read(cx).title(cx).to_string()
                }
                ActiveView::TextThread { .. } => "Text Thread".to_string(),
                _ => "".to_string(),
            };
            ...
            let tab_component = Tab::new(("agent-tab", index))
                ...
                .child(Label::new(label_text).truncate())
