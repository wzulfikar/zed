        ...
        v_flex()
            ...
            .when(render_history, |this| {
                let recent_history: Vec<_> = self.history_store.update(cx, |history_store, cx| {
                    history_store
                        .entries()
                        .take(AgentSettings::get_global(cx).max_recent_history_items)
                        .collect()
                });
    ...
    fn render_thread_controls(
        ...
        let leading_icon = if is_generating {
            if waiting_for_confirmation {
                ...
                leading_container.child(SpinnerLabel::new().size(LabelSize::Small))
            }
        } else if show_stats {
            leading_container.child(
        ...
        if waiting_for_confirmation || is_generating {
            return container;
        }
        ...
        container.child(thread_actions)
    }
